...
- **Web display**: the plugin ships a **client half** (`dsh.client.platform: "web"`, built to `lib/client.js`) that renders the hashline tool cards in the web GUI as `LINE#HASH:` — read and (enabled) grep cards in the chat row and the details panel all show the same anchors the model sees, session-scoped so only `hashline` sessions are affected. The host side also persists a faithful projection in the tool's presentation metadata (edit's web card names its fresh-anchor range), so live and replay render consistently. Replacing web cards goes through the session-scoped `tool.call.toolview` / `conversation.details.tool` slots — client-package metadata is cached, so **a `dsh web` restart is required** for the client half to load.
...
- **Sandbox-aware writes**: `edit` resolves the calling session's standing sandbox policy and threads it into the write exactly like `tool-fs` — a `danger-full-access` session can write outside the sandbox workspace, a confined session's outside-workspace write is denied, and `sandbox_permissions` + `justification` escalate a denied operation through the approval channel when the composition confines. Denials surface as the shared `[sandbox: …]` marker.
